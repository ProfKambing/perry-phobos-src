...
class Scaffold extends Module {
    ...
    Scaffold ( ) {
        ...
        this.sprint = (Setting < Boolean >) this.register ( new Setting ( "UseSprint" , true , v -> this.mode.getValue ( ) == Mode.Legit ) );
    ...
    public static
    void swap ( final int slot , final int hotbarNum ) {
    ...
    public static
    int getItemSlot ( final Container container , final Item item ) {
        int slot = 0;
        for (int i = 9; i < 45; ++ i) {
    ...
    public static
    boolean isMoving ( final EntityLivingBase entity ) {
    ...
    @SubscribeEvent
    public
    void onUpdateWalkingPlayerPost ( final UpdateWalkingPlayerEvent event ) {
    ...
    void onUpdate ( ) {
        if ( this.mode.getValue ( ) == Mode.Legit ) {
            if ( Phobos.moduleManager.isModuleEnabled ( "Sprint" ) && ( ( this.down.getValue ( ) && Scaffold.mc.gameSettings.keyBindSneak.isKeyDown ( ) ) || ! this.sprint.getValue ( ) ) ) {
    ...
    public
    double[] getExpandCoords ( final double x , final double z , final double forward , final double strafe , final float YAW ) {
    ...
    private
    BlockData getBlockData2 ( final BlockPos pos ) {
    ...
    public
    void place ( final BlockPos posI , final EnumFacing face ) {
        BlockPos pos = posI;
    ...
    class BlockData {
        ...
        public
        BlockData ( final BlockPos position , final EnumFacing face ) {

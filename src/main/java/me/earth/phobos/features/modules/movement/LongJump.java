...
class LongJump
    ...
    @SubscribeEvent
    public
    void onMove ( final MoveEvent event ) {
        if ( event.getStage ( ) != 0 ) {
            return;
        }
    ...
    @SubscribeEvent
    public
    void onTickEvent ( final TickEvent.ClientTickEvent event ) {
    ...
    @SubscribeEvent
    public
    void onUpdateWalkingPlayer ( final UpdateWalkingPlayerEvent event ) {
        if ( event.getStage ( ) != 0 ) {
            return;
        }
        if ( ! this.timer.passedMs ( this.timeout.getValue ( ) ) ) {
    ...
    private
    void doNormal ( final UpdateWalkingPlayerEvent event ) {
    ...
    private
    void doVirtue ( final MoveEvent event ) {
    ...
    private
    void updatePosition ( final double x , final double y , final double z ) {
    ...
    private
    double getDistance ( final EntityPlayer player , final double distance ) {
    ...
    private
    void setMoveSpeed ( final MoveEvent event , final double speed ) {

...
class Surround
    ...
    private final Setting < Boolean > center = this.register ( new Setting <> ( "Center" , false ) );
    ...
    private final Setting < Boolean > antiPedo = this.register ( new Setting <> ( "NoPedo" , false ) );
    private final Setting < Integer > extender = this.register ( new Setting <> ( "Extend" , 1 , 1 , 4 ) );
    ...
    private final Setting < MovementMode > movementMode = this.register ( new Setting <> ( "Movement" , MovementMode.STATIC ) );
    ...
    private final Setting < Boolean > echests = this.register ( new Setting <> ( "Echests" , false ) );
    private final Setting < Boolean > blockface = this.register ( new Setting <> ( "Block Face" , false ) );
    ...
    void doFeetPlace ( ) {
        ...
        if ( this.blockface.getValue ( ) ) {
            this.isSafe = 0;
            this.placeBlocks ( Surround.mc.player.getPositionVector ( ) , EntityUtil.getUnsafeBlockArray ( Surround.mc.player , 1 , this.floor.getValue ( ) , true ) , this.helpingBlocks.getValue ( ) , false , false );
        }

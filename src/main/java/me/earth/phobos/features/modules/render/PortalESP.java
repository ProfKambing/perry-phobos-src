...
class PortalESP
    ...
    private final Setting < Integer > distance = this.register ( new Setting <> ( "Distance" , 256 , 1 , 256 ) );
    ...
    @SubscribeEvent
    public
    void onTickEvent ( TickEvent.ClientTickEvent event ) {
    ...
    void compileDL ( ) {
        ...
        for (int x = (int) PortalESP.mc.player.posX - this.distance.getValue ( ); x <= (int) PortalESP.mc.player.posX + this.distance.getValue ( ); ++ x) {
            for (int y = (int) PortalESP.mc.player.posY - this.distance.getValue ( ); y <= (int) PortalESP.mc.player.posY + this.distance.getValue ( ); ++ y) {
                int z = (int) Math.max ( PortalESP.mc.player.posZ - (double) this.distance.getValue ( ) , 1.0 );
                while ( (double) z <= Math.min ( PortalESP.mc.player.posZ + (double) this.distance.getValue ( ) , 256.0 ) ) {

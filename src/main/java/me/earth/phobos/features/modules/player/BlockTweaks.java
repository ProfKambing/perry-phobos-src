...
class BlockTweaks
    ...
    private
    void removeGlitchBlocks ( BlockPos pos ) {
    ...
    private
    void equipBestTool ( IBlockState blockState ) {
        int bestSlot = - 1;
        double max = 0.0;
        for (int i = 0; i < 9; ++ i) {
            int eff;
    ...
    public
    void equipBestWeapon ( Entity entity ) {
        int bestSlot = - 1;
    ...
    private
    void equip ( int slot , boolean equipTool ) {
        if ( slot != - 1 ) {

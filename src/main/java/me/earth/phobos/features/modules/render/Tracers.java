...
class Tracers
    ...
    public
    double interpolate ( double now , double then ) {
        return then + ( now - then ) * (double) mc.getRenderPartialTicks ( );
    }

    public
    double[] interpolate ( Entity entity ) {
    ...
    public
    void drawLineToEntity ( Entity e , float red , float green , float blue , float opacity ) {
    ...
    public
    float[] getColorByDistance ( Entity entity ) {

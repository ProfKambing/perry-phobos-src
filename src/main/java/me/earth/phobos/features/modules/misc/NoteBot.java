...
class NoteBot
        extends Module {
    private final Setting < Boolean > tune = this.register ( new Setting <> ( "Tune" , false ) );
    private final Setting < Boolean > active = this.register ( new Setting <> ( "Active" , false ) );
    private final Setting < Boolean > downloadSongs = this.register ( new Setting <> ( "DownloadSongs" , false ) );
    private final Setting < String > loadFileSet = this.register ( new Setting <> ( "Load" , "Load File..." ) );
    private final Map < Sound, Byte > soundBytes = new HashMap <> ( );
    private final List < SoundEntry > soundEntries = new ArrayList <> ( );
    ...
    Map < Sound, BlockPos[] > setUpSoundMap ( ) {
        ...
        LinkedHashMap < Sound, BlockPos[] > result = new LinkedHashMap <> ( );
    ...
    private static
    double distanceSqToCenter ( BlockPos pos ) {
    ...
    private static
    IRegister[] createRegister ( File file ) throws IOException {
        int n2;
        FileInputStream fileInputStream = new FileInputStream ( file );
        byte[] arrby = new byte[fileInputStream.available ( )];
        fileInputStream.read ( arrby );
        ArrayList < IRegister > arrayList = new ArrayList <> ( );
        boolean bl = true;
        int n4;
        for (byte b : arrby) {
            n2 = b;
    ...
    public static
    void unzip ( File file1 , File fileIn ) {
        ZipEntry zipEntry;
    ...
    public static
    Sound getSoundFromBlockState ( IBlockState state ) {
    ...
    private
    BlockPos getAtomicBlockPos ( BlockPos blockPos ) {
    ...
    private
    void clickNoteBlock ( BlockPos pos ) {
    ...
    private
    BlockPos getRegisterPos ( SoundRegister register ) {
    ...
    class SoundRegister
        ...
        public
        SoundRegister ( Sound soundIn , byte soundByteIn ) {
            this.sound = soundIn;
    ...
    class SoundEntry {
        ...
        public
        SoundEntry ( BlockPos posIn , SoundRegister soundRegisterIn ) {
            this.pos = posIn;

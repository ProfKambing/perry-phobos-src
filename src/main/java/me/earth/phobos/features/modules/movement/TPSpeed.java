...
class TPSpeed
    ...
    private final Setting < Double > speed = this.register ( new Setting <> ( "Speed" , 0.25 , 0.1 , 10.0 ) );
    ...
    private final Setting < Boolean > turnOff = this.register ( new Setting <> ( "Off" , false ) );
    private final Setting < Integer > tpLimit = this.register ( new Setting < Object > ( "Limit" , 2 , 1 , 10 , v -> this.turnOff.getValue ( ) , "Turn it off." ) );
    private final double[] selectedPositions = {0.42 , 0.75 , 1.0};
    private int tps;
    ...
    @SubscribeEvent
    public
    void onUpdatePlayerWalking ( UpdateWalkingPlayerEvent event ) {
        if ( event.getStage ( ) != 0 ) {
            return;
        }
